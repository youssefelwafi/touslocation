...
export default function Brands() {
  ...
  function load() {
    setLoading(true);
    api.get("/marques").then((r) => setRows(r.data)).finally(() => setLoading(false));
  }
  useEffect(() => { load(); }, []);
  ...
  async function save(e) {
    ...
    try {
      if (editing) await api.put(`/marques/${editing.id}`, form);
  ...
  async function remove(r) {
    ...
    try { await api.delete(`/marques/${r.id}`); load(); }
  ...
  return (
    <div>
      <div className="page-head">
        ...
        <button className="btn-primary" onClick={openCreate}><Plus size={16} /> {t("brands.new")}</button>
      </div>
      {loading ? <Loader /> : (
        <table className="table">
          <thead><tr><th>{t("common.name")}</th><th>{t("brands.description")}</th><th>{t("brands.used")}</th><th className="actions-col">{t("common.actions")}</th></tr></thead>
          <tbody>
            {rows.map((r) => (
              <tr key={r.id}>
                <td>{r.nom}</td>
            ...
            {rows.length === 0 && <tr><td colSpan="4" className="empty">{t("brands.none")}</td></tr>}
          </tbody>
        </table>
      )}
      {open && (
        <Modal title={editing ? t("brands.edit") : t("brands.new")} onClose={() => setOpen(false)}>
